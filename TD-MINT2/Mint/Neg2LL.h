...
namespace MINT{
  ...
  public:
    ...
    */
    ...
    bool init(){
      ...
      _grad.resize(this->getParSet()->size());  
      for(unsigned int i=0; i < _grad.size(); i++) _grad[i]= 0.;
      return true;
    }
    ...
    virtual double getPdf(unsigned int evtNumber){
      ...
      //}
      /*
      if(_eventList.size() <= evtNumber){
	std::cout << "getPdf called for non-existing event number "
		  << evtNumber
		  << "while _eventList.size() = " << _eventList.size()
		  << "."
		  << std::endl;
	throw "rubbish";
      }
    ...
    virtual double getVal(){
	  ...
	  */
      ...
      for(unsigned int i=0; i < _grad.size(); i++) _grad[i]=-2. * _grad[i] * fabs(sumweights/sumsquareweights);
    ...
    virtual void Gradient(std::vector<double>& grad){
      grad=_grad;
      //for(unsigned int i=0; i < _grad.size(); i++) grad[i]=_grad[i];
    }    
    ...
    virtual ~Neg2LLClass(){}//if(0 != _grad) delete[] _grad;}
  ...
  public:
    ...
    virtual void Gradient(std::vector<double>& grad){_imini->Gradient(grad);}
