...
TH1D* createBinning(){
    ...
    NamedParameter<string> InputFile("InputFile", (std::string) "../MassFits/b2dkspi_sw_tagged.root", (char*) 0);
    NamedParameter<string> Bs_TAU_Var("B_TAU_Var",(string)"Bs_TAU");    
    NamedParameter<string> weightVar("weightVar",(string)"N_B_sw");    
    ...
    TFile* file= new TFile(((string)InputFile).c_str());
    TTree* tree= (TTree*) file->Get("DecayTree");
    tree->SetBranchStatus("*",0);
    tree->SetBranchStatus(((string)weightVar).c_str(),1);
    tree->SetBranchStatus(((string)Bs_TAU_Var).c_str(),1);
    double weight,t;
    tree->SetBranchAddress(((string)weightVar).c_str(),&weight);
...
vector< TGraph* > fitSplineAcc(string CutString, string marginalPdfsPrefix = "", double offset_dt = 0. , double scale_dt = 1.2, bool plot = false, double offset_mean_dt = 0. ){
    ...
    NamedParameter<string> Bs_TAU_Var("B_TAU_Var",(string)"B_DTF_TAU");
    NamedParameter<string> weightVar("weightVar",(string)"N_B_sw");
    ...
    tree->Add( ((string)InputFile).c_str());
    tree->SetBranchStatus("*",0);
    ...
    RooRealVar N_Bs_sw(((string)weightVar).c_str(), ((string)weightVar).c_str(), 0.);
    RooRealVar KsCat("KsCat", "KsCat", 0.);
    ...
    RooArgList observables(Bs_TAU, Bs_TAUERR, KsCat, year, N_Bs_sw, run, TriggerCat);
    RooArgList observables2(Bs_L0Global_TIS, Bs_L0HadronDecision_TOS);
...
RooFitResult * fitSplineAccRatio(string CutString, string CutStringMC, string marginalPdfsPrefix, double offset_dt, double scale_dt , double offset_dt_MC, double scale_dt_MC, double Tau, double DeltaGamma, bool plot = true, double offset_mean_dt = 0., double offset_mean_dt_MC = 0. ){
    ...
    TFile* file_mc= new TFile("/auto/data/dargent/BsDsKpipi/Final/MC/signal_18.root");
    ...
    TFile* file_norm_mc= new TFile("/auto/data/dargent/BsDsKpipi/Final/MC/norm_18.root");
...
void produceMarginalPdfs(){
    ...
    NamedParameter<string> InputFile("InputFileMarginalPdf", (std::string) "../MassFits/b2dkspi_sw_tagged.root", (char*) 0);
    TString prefix = "";
    ...
    /// Load files
    // Data
    Int_t q_OS,f,D_ID,q_SS;
    Double_t w_OS,w_SS;
    double sw;
    int run,year,KsCat,trigger;
    double t,dt;
    
    TChain* tree_norm=new TChain("DecayTree");
    tree_norm->Add( ((string)InputFile).c_str());
    tree_norm->SetBranchStatus("*",0);
    ...
    tree_norm->SetBranchStatus("run",1);
    tree_norm->SetBranchStatus("TriggerCat",1);
    tree_norm->SetBranchStatus("D_ID",1);
    ...
    tree_norm->SetBranchAddress("N_B_sw_noKs",&sw);
    ...
    tree_norm->SetBranchAddress("D_ID",&D_ID);
...
int main(int argc, char** argv){
    ...
    if(FitSplineNorm){
        ...
        //fitSplineAcc("run == 1 && KsCat == 0 && !TMath::IsNaN(B_FullDTF_TAU) && !TMath::IsNaN(B_FullDTF_TAU)" , "Run1", (double) offset_sigma_dt_Run1, (double) scale_sigma_dt_Run1, true, offset_mean_dt_Run1);
        //fitSplineAcc("run == 1 && KsCat == 1 && !TMath::IsNaN(B_FullDTF_TAU) && !TMath::IsNaN(B_FullDTF_TAUERR)" , "Run2", (double) offset_sigma_dt_Run1, (double) scale_sigma_dt_Run1, true, offset_mean_dt_Run1);
        fitSplineAcc("run == 1 && !TMath::IsNaN(B_FullDTF_TAU) && !TMath::IsNaN(B_FullDTF_TAU)" , "Run1", (double) offset_sigma_dt_Run1, (double) scale_sigma_dt_Run1, true, offset_mean_dt_Run1);
        //fitSplineAcc(" run == 2 && KsCat == 0 && !TMath::IsNaN(B_FullDTF_TAU) && !TMath::IsNaN(B_FullDTF_TAUERR)" , "Run2", (double) offset_sigma_dt_Run2, (double) scale_sigma_dt_Run2, true, offset_mean_dt_Run2);
