...
void mvas( TString fin = "TMVA.root", HistType htype = MVAType, Bool_t useTMVAStyle = kTRUE , TString outDir = "plots", bool logScale = false)
{
   ...
   while ((key = (TKey*)next())) {
      ...
      while ((titkey = (TKey*)keyIt())) {
         ...
         Float_t ymin = 0;
         Float_t maxMult = (htype == CompareType) ? 1.5 : 1.2;
         ...
         frame->GetYaxis()->SetTitle("Yield [norm.]");//"(1/N) dN^{ }/^{ }dx");
         ...
         if (htype == CompareType) {
            ...
            // perform K-S test
            cout << "--- Perform Kolmogorov-Smirnov test" << endl;
            Double_t kolS = sig->KolmogorovTest( sigOv, "D" );
            Double_t kolB = bgd->KolmogorovTest( bgdOv, "D" );
	 ...
	 /*
   	...
   	*/
         // update canvas
         if(logScale)c->GetPad(0)->SetLogy(1);
         c->Update();
         ...
         if (Save_Images) {
            if      (htype == MVAType)     TMVAGlob::imgconv( c, Form("%s/mva_%s",  outDir.Data(),   methodTitle.Data()) );
            else if (htype == ProbaType)   TMVAGlob::imgconv( c, Form("%s/proba_%s",  outDir.Data(), methodTitle.Data()) ); 
            else if (htype == CompareType) TMVAGlob::imgconv( c, Form("%s/overtrain_%s",outDir.Data(), methodTitle.Data()) ); 
            else                           TMVAGlob::imgconv( c, Form("%s/rarity_%s",  outDir.Data(),methodTitle.Data()) ); 
         }
         countCanvas++;
